...
function RefreshToken(user){return jwt.sign({ username: user }, REFRESH_TOKEN_SECRET);}


function AccessToken(user){return jwt.sign({ username: user }, ACCESS_TOKEN_SECRET, { expiresIn: '15m' });}

function verify(refreshToken){return jwt.verify(refreshToken, REFRESH_TOKEN_SECRET);}
