...
export function emitRegister(username,password){
    socket.emit('Register',{"RequestMetaData":{username,password}})
}
export function emitLogin(username,password){
    socket.emit('Login',{"RequestMetaData":{username,password}})
}
...
export function setupSocketConnection(){
    ...
    socket.on("redirect",async (response)=>{
        localStorage.setItem('accessToken', response.RequestMetaData.accessToken);
    ...
    socket.on("failedReg",async (response)=>{
...
export function getUserTileData(){
    const username = localStorage.getItem("username");
